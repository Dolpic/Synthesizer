...
from Modules.Filters.Biquad.PeakingEQ import *
...
class Resonator(Module):
    ...
    def __init__(self, dampening, freq_mult=1, freq_add=0, max=100000):
        super().__init__()
        self.dampening, self.freq_mult, self.freq_add, self.max = self._param_to_modules([dampening, freq_mult, freq_add, max])
    ...
    def get(self, indexes, freq_amp):
        ...
        for freq, amp in freq_amp:
            initial_freq = freq
            count = 0
            while amp > self.audible_limit and count <= self.max.get(indexes, indexes)[0]:
